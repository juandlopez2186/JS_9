...
function agregarEvento() {
    ...
    if (diferencia(fechaEvento.value)<0) {
        return
    }
...
function diferencia(destino) {
    let fechaDestino = new Date(destino);
    let fechaActual = new Date();
    let diferenciaF= fechaDestino.getTime() - fechaActual.getTime()
    let dias = Math.ceil(diferenciaF / (1000 * 3600 * 24))//milisegundos => horas => Dias
    return dias
}
function mostrarEventos() {
    const eventosHTML=eventos.map((evento)=>{
        ...
        <div class="evento">
            <div class="dias">
                <span class="diasfaltantes">${diferencia(evento.fecha)}</span><br>
